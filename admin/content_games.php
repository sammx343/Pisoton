 <?php
  ...
  if( isset($_GET['game-delete']) )
  {
      ...
      if(isset($_GET['game']))
      {
          ...
          $query = "DELETE FROM Pis_juegos WHERE $condition";
    ...
    if(isset($_GET['game']))
    {
      ...
      $query0 = "SELECT * FROM Pis_juegos WHERE idJuegos = :idGame ";
      $parametros0 = array(':idGame'=> $idGame);
      $rows = $pdoconnector->executeQuery($query0,$parametros0);
      if(count($rows)>0){
        $row = $rows[0];
        if($row  != '') {
          $idEntry = $row['idJuegos'];
          $title =  $row['title'];
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    ...
                    <div class="col-lg-12">
                        <h1 class="page-header">Juegos</h1>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                <!-- /.row -->
    ...
    <script type="text/javascript">
        ...
        $("#add-content").click(function()
        {
            if ($("#add_title").val() != '' && $("#add_urlImage").val() != '' && $("#add_description").val() != '' && $("#add_url").val() != '')
            {
                ...
                $.ajax({
                    type: "POST",
                    url: "register_game.php",
                    data:{idGame:idEntry,title:title,urlImage:urlImage,description:description,url:url},
