<?php
  ...
  if( isset($_GET['trailer-delete']) )
  {
      ...
      if(isset($_GET['trailer']))
      {
          ...
          {
              $condition.= "idTrailers=$color or ";
              //echo $color."<br />";
          }
          $condition.= "idTrailers=-1";

          $connection = mysql_connect($db_host,$db_user,$db_password);
          mysql_select_db($db_schema,$connection);
          mysql_set_charset('utf8',$connection);

          $query = "DELETE FROM Pis_trailers WHERE $condition";
          $result = mysql_query($query,$connection);
          if($result)
          {
            ?>
              <p class="bg-success text-center">El(Los) trailer(s) ha(n) sido eliminado(s)</p>
              <?php
            }
          else
          {
      ...
      {
        ?>
          <p class="bg-danger text-center">No has seleccionado un trailer</p>
        <?php
      }
  }
?>
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
                ...
                <div class="row">
                    <div class="col-lg-12">
                         <ul class="nav nav-tabs" role="tablist">
                            <li role="presentation" class="active"><a href="#add_trailer" aria-controls="add_trailer" role="tab" data-toggle="tab">Agregar Trailer</a></li>
                            <li role="presentation"><a href="#all_trailers" aria-controls="all_trailers" role="tab" data-toggle="tab">Todos los Trailers</a></li>
                          </ul>


                          <div class="tab-content">
                            <div role="tabpanel" class="tab-pane active" id="add_trailer">
                                <div>
                                    <div class="margenes">
                                        ...
                                        <div class="btnestilOferta">
                                            <a class="btn btn-success" id="add-trailer" data-toggle="tab" >Agregar Contenido</a>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div role="tabpanel" class="tab-pane" id="all_trailers">
                                <div class="margenes">
                                    <?php
                                        require('lib/config.php');
                                        $connection = mysql_connect($db_host,$db_user,$db_password);
                                        ...
                                        $query = "SELECT * FROM Pis_trailers";
                                        $result = mysql_query($query,$connection);
                                    ...
                                    <input class="btn btn-danger" type="submit" name="trailer-delete" id="trailer-delete" value="Eliminar">
                                    <div class="table-responsive" style="margin-top:40px;">
                                        <table class="table" id="table_all_trailers">
                                            <thead>
                                                <tr>
                                            ...
                                            ?>
                                              <tr class="item_list">
                                                <td class="text-center idCheck" id="<?php echo $row['idTrailers']; ?>"><input type="checkbox" name="trailer[]" id="trailer[]" value="<?php echo $row['idTrailers']; ?>" /></td>
                                                ...
                                                <td class="text-center edit-dateTrailer"><?php echo $row['dateTrailer']; ?></td>
                                              </tr>
                                            <?php
                                              }
                                            ?>
              ...
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <div class="modal-body center-block">
                    <p>Registrando trailer en la Bases de datos</p>
            ...
            <div class="modal fade" id="editTrailerModal" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-sm" role="document">
                  <div class="modal-content">
                    <div class="modal-header">
                      <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                      <h4 class="modal-title" id="exampleModalLabel">Edición Trailer</h4>
                    </div>
    ...
    <script type="text/javascript">
        ...
        $("#editTrailerModal").on( "click", "#edit-trailer", function() {
          $('#editTrailerModal').modal('hide');
            if ($("#edit_title").val() != '' && $("#edit_description").val() != '' && $("#edit_url").val() != '')
            {
                $('#myModal').modal('show');
                ...
                var formData = new FormData();
                formData.append('image', image);
                formData.append('idTrailers', idEntry);
                formData.append('description', description);
                formData.append('title', title);
                formData.append('url', url);
                $.ajax({
                    ...
                    success: function(mensaje)
                    {
                        ...
                        if(obj.exito == "1")
                        {
                          ...
                          $("#"+obj.datos[':idTrailers']).siblings(".edit-title").html(obj.datos[':title']);
                          $("#"+obj.datos[':idTrailers']).siblings(".edit-description").html(obj.datos[':description']);
                          $("#"+obj.datos[':idTrailers']).siblings(".edit-url").html(obj.datos[':url']);
                          $("#"+obj.datos[':idTrailers']).siblings(".edit-dateTrailer").html(obj.datos[':dateTrailer']);
                          $("#"+obj.datos[':idTrailers']).siblings(".edit-image").children('img').attr('src','uploads/'+obj.datos[':urlImage']);
                        }
                        else
                        {
                            $('#prueba').modal('show');
        ...
        $("#all_trailers").on( "click", ".edit-entry", function() {
          ///alert("click");
          ...
          $("#editTrailerModal .modal-body").html("<div class='margenes'>  \
                                                    <p>(*) Campos obligatorios</p>  \
                                                    <div class='form-group'>  \
                                                    ...
                                                    <div class='btnestilOferta'>  \
                                                        <a class='btn btn-success' id='edit-trailer' data-toggle='tab' >Editat Trailer</a>  \
                                                    </div>  \
                                                </div> "
                                              )
        ...
        $('#editTrailerModal').on('click','#triggerFileEdit', function(e) {
          e.preventDefault();
